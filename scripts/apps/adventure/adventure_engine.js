const TextAdventureEngine = (() => {
    ...
    const defaultVerbs = {
        ...
        go: {
            action: 'go',
            aliases: ['north', 'south', 'east', 'west', 'up', 'down', 'n', 's', 'e', 'w', 'u', 'd', 'enter', 'exit']
        },
    ...
    function startAdventure(adventureData, options = {}) {
        ...
        const gamePromise = TextAdventureModal.show(adventure, {
            processCommand,
            onExitRequest: TextAdventureModal.hide
        }, scriptingContext);
        _displayCurrentRoom();
        return gamePromise;
    }
    ...
    async function processCommand(command) {
        ...
        for (const cmd of parsedCommands) {
            ...
            switch (verb.action) {
                case 'look':
                    _handleLook(directObject, onDisambiguation);
                    break;
                case 'go':
                    _handleGo(directObject);
                    break;
                case 'take':
                    _handleTake(directObject, onDisambiguation);
                    break;
                case 'drop':
                    _handleDrop(directObject, onDisambiguation);
                    break;
                case 'use':
                    _handleUse(directObject, indirectObject, onDisambiguation);
                    break;
                case 'open':
                    _handleOpen(directObject, onDisambiguation);
                    break;
                case 'close':
                    _handleClose(directObject, onDisambiguation);
                    break;
                case 'unlock':
                    _handleUnlock(directObject, indirectObject, onDisambiguation);
                    break;
                case 'inventory':
                    _handleInventory();
                    break;
                ...
                case 'quit':
                    ...
                    break;
                case 'save':
                    await _handleSave(directObject);
                    break;
                case 'load':
                    await _handleLoad(directObject);
                    break;
                case 'talk':
                    _handleTalk(directObject, onDisambiguation);
                    break;
                case 'ask':
                    _handleAsk(directObject, indirectObject, onDisambiguation);
                    break;
                case 'give':
                    _handleGive(directObject, indirectObject, onDisambiguation);
                    break;
                case 'show':
                    _handleShow(directObject, indirectObject, onDisambiguation);
                    break;
                case 'score':
                    _handleScore();
                    break;
                case 'read':
                    _handleRead(directObject, onDisambiguation);
                    break;
                case 'eat':
                    _handleEatDrink('eat', directObject, onDisambiguation);
                    break;
                case 'drink':
                    _handleEatDrink('drink', directObject, onDisambiguation);
                    break;
                case 'push':
                    _handlePushPullTurn('push', directObject, onDisambiguation);
                    break;
                case 'pull':
                    _handlePushPullTurn('pull', directObject, onDisambiguation);
                    break;
                case 'turn':
                    _handlePushPullTurn('turn', directObject, onDisambiguation);
                    break;
                case 'wear':
                    _handleWearRemove('wear', directObject, onDisambiguation);
                    break;
                case 'remove':
                    _handleWearRemove('remove', directObject, onDisambiguation);
                    break;
                case 'listen':
                    _handleSensoryVerb('listen', directObject, onDisambiguation);
                    break;
                case 'smell':
                    _handleSensoryVerb('smell', directObject, onDisambiguation);
                    break;
                case 'touch':
                    _handleSensoryVerb('touch', directObject, onDisambiguation);
                    break;
                case 'wait':
                    _handleWait();
                    break;
                case 'dance':
                    TextAdventureModal.appendOutput("You do a little jig. You feel refreshed.", 'system');
                    break;
                case 'sing':
                    TextAdventureModal.appendOutput("You belt out a sea shanty. A nearby bird looks annoyed.", 'system');
                    break;
                case 'jump':
                    TextAdventureModal.appendOutput("You jump on the spot. Whee!", 'system');
                    break;
                case 'light':
                    _handleLight(directObject, onDisambiguation);
                    break;
                default:
    ...
    function _handleGive(itemTarget, npcTarget, onDisambiguation) {
        ...
        if (itemResult.found.length > 1) {
            disambiguationContext = {
                ...
                context: {callback: (item) => _handleGive(item.name, npcTarget, onDisambiguation)}
            };
        ...
        if (npcResult.found.length > 1) {
            disambiguationContext = {
                found: npcResult.found,
                context: {callback: (npc) => _performGive(itemToGive, npc)}
            };
    ...
    function _handleShow(itemTarget, npcTarget, onDisambiguation) {
        ...
        if (itemResult.found.length > 1) {
            disambiguationContext = {
                found: itemResult.found,
                context: {callback: (item) => _handleShow(item.name, npcTarget, onDisambiguation)}
            };
            const itemNames = itemResult.found.map(i => i.name).join(' or the ');
        ...
        if (npcResult.found.length > 1) {
            disambiguationContext = {
                found: npcResult.found,
                context: {callback: (npc) => _performShow(itemToShow, npc)}
            };
            const npcNames = npcResult.found.map(n => n.name).join(' or the ');
    ...
    function _handlePushPullTurn(verb, target, onDisambiguation) {
        ...
        if (result.found.length > 1) {
            disambiguationContext = {
                found: result.found,
                context: {callback: (item) => _performPushPullTurn(verb, item)}
            };
            const itemNames = result.found.map(item => item.name).join(' or the ');
            TextAdventureModal.appendOutput(`Which do you want to ${verb}, the ${itemNames}?`, 'info');
            onDisambiguation();
    ...
    function _handleWearRemove(verb, target, onDisambiguation) {
        ...
        if (result.found.length > 1) {
            disambiguationContext = {
                found: result.found,
                context: {callback: (item) => _performWearRemove(verb, item)}
            };
            const itemNames = result.found.map(item => item.name).join(' or the ');
    ...
    function _handleSensoryVerb(verb, target, onDisambiguation) {
        ...
        if (result.found.length > 1) {
            disambiguationContext = {
                found: result.found,
                context: {callback: (item) => _performSensoryVerb(verb, item)}
            };
            const itemNames = result.found.map(item => item.name).join(' or the ');
