...
class PaintManager extends App {
    constructor() {
        super();
        this.state = {};
    ...
    enter(appLayer, options = {}) {
        ...
        this.state = this._createInitialState(options.filePath, options.fileContent);
        this.isActive = true;

        this.container = PaintUI.buildAndShow(this.state, this.callbacks);
    ...
    _createInitialState(filePath, fileContent) {
        ...
        if (initialState.canvasData.length === 0) {
            initialState.canvasData = this._getBlankCanvas(initialState.canvasDimensions);
        }
    ...
    _createCallbacks() {
        return {
            ...
            onUndo: () => {
                ...
                this.state.canvasData = JSON.parse(this.state.undoStack[this.state.undoStack.length - 1]);
                PaintUI.renderCanvas(this.state.canvasData, this.state.canvasDimensions);
            ...
            onCanvasMouseDown: (coords) => {
                ...
                if (this.state.currentTool === 'fill') {
                    ...
                    const fillCells = this._getCellsForFill(coords.x, coords.y, color, char);
            ...
            onCanvasMouseMove: (coords) => {
                ...
                if (this.state.isDrawing) {
                    switch (this.state.currentTool) {
                        case 'select':
                            const { x, y, width, height } = this._getSelectionRect(this.state.startCoords, coords);
                            PaintUI.showSelectionRect({ x, y, width, height });
                            break;
                        case 'line':
                            previewCells = this._getCellsForLine(this.state.startCoords.x, this.state.startCoords.y, coords.x, coords.y, char, color);
                            break;
                        case 'rect':
                            previewCells = this._getCellsForRect(this.state.startCoords.x, this.state.startCoords.y, coords.x, coords.y, char, color);
                            break;
                        case 'circle':
                            const rx = Math.abs(this.state.startCoords.x - coords.x);
                            const ry = Math.abs(this.state.startCoords.y - coords.y);
                            previewCells = this._getCellsForEllipse(this.state.startCoords.x, this.state.startCoords.y, rx, ry, char, color);
                            break;
                        case 'pencil':
                        case 'eraser':
                            const cells = this._getCellsForLine(this.state.startCoords.x, this.state.startCoords.y, coords.x, coords.y, char, color);
            ...
            onCanvasMouseUp: () => {
                ...
                if (this.state.currentTool === 'select' && this.state.startCoords) {
                    this.state.selection = this._getSelectionRect(this.state.startCoords, endCoords);
                } else if (this.state.currentTool !== 'select' && this.state.startCoords && endCoords) {
                    const char = this.state.currentTool === 'eraser' ? ' ' : this.state.currentCharacter;
                    const color = this.state.currentTool === 'eraser' ? '#000000' : this.state.currentColor;
                    let finalCells = [];
                    ...
                    switch (this.state.currentTool) {
                        case 'line':
                            finalCells = this._getCellsForLine(this.state.startCoords.x, this.state.startCoords.y, endCoords.x, endCoords.y, char, color);
                            break;
                        case 'rect':
                            finalCells = this._getCellsForRect(this.state.startCoords.x, this.state.startCoords.y, endCoords.x, endCoords.y, char, color);
                            break;
                        case 'circle':
                            const rx = Math.abs(this.state.startCoords.x - endCoords.x);
                            const ry = Math.abs(this.state.startCoords.y - endCoords.y);
                            finalCells = this._getCellsForEllipse(this.state.startCoords.x, this.state.startCoords.y, rx, ry, char, color);
                            break;
                    }
            ...
            onPaste: () => {
                ...
                for (let i = 0; i < this.state.clipboard.length; i++) {
                    for (let j = 0; j < this.state.clipboard[i].length; j++) {
                        pastedCells.push({ x: pasteX + j, y: pasteY + i, ...this.state.clipboard[i][j] });
                    }
                }
    ...
    _applyCellsToData(cells) {
        cells.forEach(cell => {
            if (cell.y >= 0 && cell.y < this.state.canvasDimensions.height && cell.x >= 0 && cell.x < this.state.canvasDimensions.width) {
    ...
    async _saveContent() {
        ...
        const saveResult = await FileSystemManager.createOrUpdateFile(this.state.currentFilePath, jsonContent, { currentUser, primaryGroup });

        if (saveResult.success && await FileSystemManager.save()) {
            this.state.isDirty = false;
        } else {
            PaintUI.updateStatusBar({ ...this.state, statusMessage: `Error: ${saveResult.error || "Failed to save to filesystem."}` });
        }
        PaintUI.updateToolbar(this.state);
    }
    _getCellsInBrush(x, y, char, color) {
        ...
        for (let i = 0; i < this.state.brushSize; i++) {
            for (let j = 0; j < this.state.brushSize; j++) {
                ...
                if (drawY >= 0 && drawY < this.state.canvasDimensions.height && drawX >= 0 && drawX < this.state.canvasDimensions.width) {
